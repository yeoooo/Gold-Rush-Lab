...
public class MiningFailureClassifier {
    ...
    /**
     * 채굴 처리 중 발생한 예외를 메트릭용 실패 유형으로 분류하는 함수.
     *
     * 1. 최상위 예외부터 마지막 원인까지 예외 체인을 수집한다.
     * 2. 사용자 없음과 광산 고갈 업무 예외를 확인한다.
     * 3. deadlock, timeout, lock 획득 실패 여부를 순서대로 확인한다.
     * 4. 그 밖의 데이터 접근 예외를 DB 오류로 분류한다.
     * 5. 어떤 조건에도 해당하지 않으면 UNKNOWN을 반환한다.
     *
    ...
    public MiningFailureType classify(Throwable throwable) {
        ...
        if (containsType(causes, UserNotFoundException.class)) {
            return MiningFailureType.USER_NOT_FOUND;
        }
        if (containsType(causes, MineDepletedException.class)) {
            return MiningFailureType.MINE_DEPLETED;
        }
        ...
        if (containsType(causes, DataAccessException.class)) {
            return MiningFailureType.DATABASE;
        }
